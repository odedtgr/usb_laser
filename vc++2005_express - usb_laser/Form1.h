...
#include <cstdio>
#include <string>		//definitions needed for the SetupDixxx() functions, which we use to find our plug and 
...
#pragma region Global Variables
	...
	BOOL ToggleLEDsPending = FALSE;	
	BOOL SetLaserPowerPending = FALSE;		//Updated by ToggleLED(s) button click event handler, used by ReadWriteThread (needs to be atomic)
	double laserPowerTarget = 0; //precentage of max laser power 
	double laserPowerCurrent = 0;//precentage of max laser power
	int laserPowerMax = 1023; //maximal power in PWM counts
	int laserPowerOffset = 370; //offset (minimal laser emission) in PWM counts
...
private: System::Windows::Forms::TextBox^  laserPowerTb;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->laserPowerTb = (gcnew System::Windows::Forms::TextBox());
			this->SuspendLayout();
			// 
			// StatusBox_lbl
			// 
			...
			this->StatusBox_lbl->Size = System::Drawing::Size(41, 15);
			...
			this->PushbuttonState_lbl->Size = System::Drawing::Size(158, 15);
			...
			// laserPowerTb
			// 
			this->laserPowerTb->Location = System::Drawing::Point(87, 98);
			this->laserPowerTb->Margin = System::Windows::Forms::Padding(2);
			this->laserPowerTb->Name = L"laserPowerTb";
			this->laserPowerTb->Size = System::Drawing::Size(76, 20);
			this->laserPowerTb->TabIndex = 20;
			this->laserPowerTb->KeyDown += gcnew System::Windows::Forms::KeyEventHandler(this, &Form1::laserPowerTb_KeyDown);
			// 
			// Form1
			// 
			...
			this->Controls->Add(this->laserPowerTb);
...
//-------------------------------------------------------BEGIN CUT AND PASTE BLOCK-----------------------------------------------------------------------------------
		...
		while(true)
		{
			if(AttachedState == TRUE)	//Do not try to use the read/write handles unless the USB device is attached and ready
			{
				...
				//Check if this thread should send a SetLaserPower command to the firmware.  SetLaserPowerPending will get set
				//by the laserPowerTb_KeyDown event handler function if the user Enter on the Laser Power Textbox.
				if (SetLaserPowerPending == TRUE)
				{
					int laserPower = (int)(laserPowerTarget / 100 * (laserPowerMax - laserPowerOffset) + laserPowerOffset+0.5);//convert laser power from precentage to PWM counts
					OUTBuffer[0] = 0;				//The first byte is the "Report ID" and does not get sent over the USB bus.  Always set = 0.
					OUTBuffer[1] = 0x90;			//0x80 is the "SET_LASER_POWER" command in the firmware
					OUTBuffer[2] = laserPower & 0xFF;             //LaserPowerTarget LSB
					OUTBuffer[3] = laserPower >> 8;            //LaserPowerTarget MSB
					for (unsigned char i = 4; i < 65; i++)	//This loop is not strictly necessary.  Simply initializes unused bytes to
						OUTBuffer[i] = 0xFF;				//0xFF for lower EMI and power consumption when driving the USB cable.
					//Now send the packet to the USB firmware on the microcontroller
					WriteFile(WriteHandleToUSBDevice, &OUTBuffer, 65, &BytesWritten, 0);	//Blocking function, unless an "overlapped" structure is used
					SetLaserPowerPending = FALSE;
				}
...
private: System::Void laserPowerTb_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
	if (e->KeyCode == Keys::Enter)
	{
		//TODO: cast laser power string to int and send to microcontroller
		String^ laserPowerText = laserPowerTb->Text;
		laserPowerTarget = System::Int32::Parse(laserPowerText);
		SetLaserPowerPending = TRUE;
	}
}
};
}
